...
public:
    ...
    std::string getInputDir() override {
        return TestHelper::appendRag3dbRootPath("dataset/node-insertion-deletion-tests/int64-pk/");
    }
...
TEST_F(NodeInsertionDeletionTests, InsertManyNodesTest) {
    ...
    for (int64_t i = 0; i < (int64_t)RAG3DB_PAGE_SIZE; i++) {
        auto result =
    ...
    ASSERT_EQ(tuple->getValue(0)->getValue<int64_t>(), RAG3DB_PAGE_SIZE);
