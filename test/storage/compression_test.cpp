...
TEST(CompressionTests, IntegerBitpackingMetadataInvalidPhysicalType) {
    ...
    rag3db::storage::InPlaceUpdateLocalState localUpdateState{};
    EXPECT_THROW(metadata.canUpdateInPlace(&data, 0, 1, PhysicalTypeID::ARRAY, localUpdateState),
        StorageException);

    EXPECT_THROW(metadata.numValues(RAG3DB_PAGE_SIZE, LogicalType::STRING()), StorageException);
...
TEST(CompressionTests, FloatCompressionMetadataInvalidPhysicalType) {
    ...
    EXPECT_THROW(metadata.numValues(RAG3DB_PAGE_SIZE, LogicalType::UINT64()), StorageException);
...
TEST(CompressionTests, IntegerPackingTest128AllPositive) {
    std::vector<rag3db::common::int128_t> src(101);

    {
        rag3db::common::int128_t cur = 1;
        rag3db::common::int128_t diff = 1;
...
TEST(CompressionTests, IntegerPackingTest128Negative) {
    ...
    {
        auto cur = -(rag3db::common::int128_t(1) << 120);
        rag3db::common::int128_t diff = 1;
        std::ranges::generate(src.begin(), src.end(), [&diff, &cur] {
            diff *= 2;
            cur += diff;
...
TEST(CompressionTests, IntegerPackingTest128NegativeSimple) {
    std::vector<rag3db::common::int128_t> src{-1024, -1027, -1023};
...
TEST(CompressionTests, IntegerPackingTest128CompressFullChunkLargeWidth) {
    std::vector<rag3db::common::int128_t> src{IntegerBitpacking<int128_t>::CHUNK_SIZE,
...
TEST(CompressionTests, IntegerPackingMultiPage64) {
    ...
    std::vector<rag3db::common::int128_t> src(numValues);
    const auto M = (rag3db::common::int128_t(1) << 126) - 1;
