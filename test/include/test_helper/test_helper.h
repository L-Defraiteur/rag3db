...
public:
    ...
    static constexpr uint64_t HASH_INDEX_MEM =
        common::HashIndexConstants::NUM_HASH_INDEXES * common::RAG3DB_PAGE_SIZE * 9;
    ...
    static std::string getTestListFile() {
        return appendRag3dbRootPath(std::string(E2E_TEST_FILES_DIRECTORY) + "/test_list");
    }

    static std::string appendRag3dbRootPath(const std::string& path) {
        if (std::filesystem::path(path).is_relative()) {
            return RAG3DB_ROOT_DIRECTORY + std::string("/") + path;
        }
        return path;
    }
    ...
    static std::filesystem::path getRootTempDir() {
        ...
        if (tempDir != nullptr) {
            return std::filesystem::path(tempDir) / "rag3db";
        }
        return std::filesystem::temp_directory_path() / "rag3db";
    }
...
#ifdef _WIN32
        // rag3db still doesn't support backslashes in paths on windows
