...
TEST_F(PrivateApiTest, CloseConnectionWithActiveTransaction) {
    ...
    conn.reset();
    conn = std::make_unique<rag3db::main::Connection>(database.get());
    conn->query("BEGIN TRANSACTION;");
...
TEST_F(EmptyDBTransactionTest, DatabaseFilesAfterCheckpoint) {
    ...
    ASSERT_FALSE(
        std::filesystem::exists(rag3db::storage::StorageUtils::getTmpFilePath(databasePath)));
    ASSERT_FALSE(
        std::filesystem::exists(rag3db::storage::StorageUtils::getShadowFilePath(databasePath)));
    ASSERT_FALSE(
        std::filesystem::exists(rag3db::storage::StorageUtils::getWALFilePath(databasePath)));
    conn->query("CREATE NODE TABLE test(id INT64 PRIMARY KEY, name STRING);");
    ASSERT_TRUE(std::filesystem::exists(rag3db::storage::StorageUtils::getWALFilePath(databasePath)));
...
static void insertNodes(uint64_t startID, uint64_t num, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
...
static void insertRelationships(uint64_t startID, uint64_t num, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
...
static void updateNodes(uint64_t startID, uint64_t num, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
    for (uint64_t i = 0; i < num; ++i) {
        auto id = startID + i;
...
static void updateMixedTypeNodes(uint64_t startID, uint64_t num, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
        auto id = startID + i;
...
static void updateRelationships(uint64_t startID, uint64_t num, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
    for (auto i = 0u; i < num; ++i) {
        auto fromID = startID + i;
        auto toID = (startID + i + 1) % (num * 4);
...
static void updateRelationshipsWithMixedTransactions(uint64_t startID, uint64_t num,
    bool shouldCommit, rag3db::main::Database& database) {
    auto conn = std::make_unique<rag3db::main::Connection>(&database);
    conn->query("BEGIN TRANSACTION;");
    for (auto i = 0u; i < num; ++i) {
        auto fromID = startID + i;
