...
TestParser::TestParser(std::string path)
    ...
    variableMap.insert({"RAG3DB_ROOT_DIRECTORY", Value(RAG3DB_ROOT_DIRECTORY)});
    variableMap.insert({"RAG3DB_VERSION", Value(RAG3DB_VERSION)});
    variableMap.insert({"RAG3DB_EXPORT_DB_DIRECTORY", Value(exportDBPath)});
}
...
void TestParser::genGroupName() const {
    const std::size_t subStart =
        TestHelper::appendRag3dbRootPath(std::string(TestHelper::E2E_TEST_FILES_DIRECTORY)).length() +
        1;
...
void TestParser::extractDataset() {
    ...
    if (datasetType == "PARQUET") {
        ...
        testGroup->dataset = currentToken.params[2];
    } else if (datasetType == "RAG3DB") {
        testGroup->datasetType = TestGroup::DatasetType::RAG3DB;
        testGroup->dataset = currentToken.params[2];
...
TestQueryResult TestParser::extractExpectedResultFromToken() {
    ...
    if (result == "ok") {
        ...
        if (line.starts_with("<FILE>:")) {
            ...
            queryResult.expectedResult.push_back(TestHelper::appendRag3dbRootPath(
...
TestStatement TestParser::parseStatement(const std::string& testCaseName) {
    ...
    while (true) {
        ...
        case TokenType::BATCH_STATEMENTS: {
            std::string query = paramsToString(1);
            extractConnName(query, statement);
            statement.batchStatementsCSVFile = TestHelper::appendRag3dbRootPath(
    ...
    while (nextLine()) {
        ...
        case TokenType::LOAD_DYNAMIC_EXTENSION: {
            ...
            loadExtensionStatement.query =
                stringFormat("LOAD EXTENSION '{}/extension/{}/build/lib{}.rag3db_extension'",
                    RAG3DB_ROOT_DIRECTORY, extensionName, extensionName);
