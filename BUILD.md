...
│
├── extension/tantivy/ld-tantivy/          <- Rust : lib Tantivy (cargo workspace)
│   └── tantivy_fts/rust/                  <- Rust : crate FFI cxx bridge
│       └── target/release/libtantivy_fts.a  <- sortie cargo
│
├── extension/tantivy_fts/                 <- C++ : extension rag3db
│   ├── build/libtantivy_fts.rag3db_extension  <- shared lib (LOAD EXTENSION)
│   └── test/tantivy_fts_test             <- binaire test GTest
```
...
Voir [`extension/tantivy_fts/BUILD.md`](extension/tantivy_fts/BUILD.md) pour le detail complet.
...
Sortie : `tools/nodejs_api/build/rag3dbjs.node` + `extension/tantivy_fts/build/libtantivy_fts.rag3db_extension`
...
Extensions liees statiquement : tantivy_fts, json, vector, algo.
...
| Tests passent mais avec vieux scores | Extension `.rag3db_extension` pas re-linkee | `cmake --build . --target rag3db_tantivy_fts_extension` |
