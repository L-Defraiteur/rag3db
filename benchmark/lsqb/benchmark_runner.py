...
from serializer import _get_rag3db_version
...
# import rag3db Python API
sys.path.append(os.path.join(base_dir, "..", ".."))
import tools.python_api.build.rag3db as rag3db
...
def run_rag3db(sf, serialized_graph_path, num_threads):
    db = rag3db.Database(os.path.join(serialized_graph_path, "db.rag3db"))
    conn = rag3db.Connection(db, num_threads=num_threads)
    if timeout is not None:
        ...
        for i in range(1, 10):
            ...
            with open(
                ...
                try:
                    ...
                    results_file.write(
                        f"Rag3dbDB\t{i}\t{num_threads} threads\t{sf}\tTimeout\tNA\tNA\t\n"
                    )
                    results_file.flush()
                    continue
                ...
                # Upload the result
                results_file.write(
                    f"Rag3dbDB\t{i}\t{num_threads} threads\t{sf}\t{execution_time / 1000:.4f}\t{memory / (1024 ** 3):.2f} GB\t{result[0]}\n"
                )
                results_file.flush()
...
if __name__ == "__main__":
    ...
    logging.info("Database version: %s", _get_rag3db_version())
    ...
    run_rag3db(scale_factor, serialized_graphs_path[dataset_name], threads)
