...
from version import _get_rag3db_version

base_dir = os.path.dirname(os.path.realpath(__file__))

def serialize(rag3db_exec_path, dataset_name, dataset_path, serialized_graph_path, benchmark_copy_log_dir,
              single_thread: bool = False):
    bin_version = _get_rag3db_version()
    ...
    for s in serialize_queries:
        ...
        # Run rag3db shell one query at a time. This ensures a new process is
        ...
        process = subprocess.Popen([rag3db_exec_path, db_path],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Serializes dataset to a rag3db database')
    ...
    if sys.platform == "win32":
        default_rag3db_exec_path = os.path.join(
            base_dir, '..', 'build', 'release', 'tools', 'shell', 'rag3db_shell')
    else:
        default_rag3db_exec_path = os.path.join(
            base_dir, '..', 'build', 'release', 'tools', 'shell', 'rag3db')
    parser.add_argument("--rag3db-shell",
                        help="Path of the rag3db shell executable. Defaults to the path as built in the default release build directory",
                        default=default_rag3db_exec_path)
    args = parser.parse_args()

    try:
        serialize(args.rag3db_shell, args.dataset_name, args.dataset_path, args.serialized_graph_path,
