...
Écrire des tests E2E automatisés en C++ (GTest) pour l'extension lucivy_fts, suivant le pattern des extensions existantes (FTS, vector, algo).

## Fichiers créés/modifiés

| Fichier | Action |
|---------|--------|
| `extension/lucivy_fts/test/CMakeLists.txt` | **Créé** — garde `BUILD_EXTENSION_TESTS` |
| `extension/lucivy_fts/test/lucivy_fts_test.cpp` | **Créé** — 4 tests GTest |
| `extension/lucivy_fts/CMakeLists.txt` | **Modifié** — ajouté `add_subdirectory(test)` |
...
- `QUERY_LUCIVY_INDEX` est un `TableFunc` (pas standalone) → nécessite `RETURN node_id, score, highlights`

## Les 4 tests

### LucivyCreateAndQueryTest
...
2. `CALL DROP_LUCIVY_INDEX('doc')` → succès
...
- **`cmake --build . --target lucivy_fts_test`** : build ciblé car le build global échoue sur `gtest_discover_tests` des autres targets (api_test, c_api_test) à cause du même problème GLIBCXX.
