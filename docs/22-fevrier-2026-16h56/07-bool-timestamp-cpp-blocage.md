...
- `mapLogicalTypeToTantivy()` : ajout `LogicalTypeID::BOOL → "i64"`, `LogicalTypeID::TIMESTAMP → "i64"`
...
### tantivy_fts_test.cpp
- Nouveau test `TantivyBoolTimestampFilterTest` : table avec BOOLEAN + TIMESTAMP, filtres eq/gt/lt

## Le problème : tantivy_index.cpp (insert / update / finalize)

`tantivy_index.cpp` a 3 endroits qui lisent les valeurs des filter fields :
1. `insert()` (ligne ~220) — appelé par les hooks INSERT
2. `finalize()` (ligne ~360) — checkpoint
3. `update()` appelle `insert()` en interne → couvert

Le problème : quand `tantivyType == "i64"`, le code fait `getValue<int64_t>(pos)`. Mais si la colonne est BOOLEAN (PhysicalTypeID::BOOL, 1 byte), il faut `getValue<bool>(pos)` puis convertir en 0/1.
...
- `create_tantivy_index.cpp` (ligne ~415) : déjà collecte les types, les passer au constructeur
- `tantivy_index.cpp` : utiliser `indexInfo.columnTypes[f]` pour BOOL check
...
### Option B : Cache `physicalTypes_` dans TantivyIndex (~15 lignes)

**Fichier :** `tantivy_index.h` + `tantivy_index.cpp`

Ajouter un `std::vector<PhysicalTypeID> physicalTypes_` dans TantivyIndex. Le remplir dans le constructeur ET dans `load()` en relisant les types depuis le table catalog entry.

```cpp
// Dans le constructeur TantivyIndex (après les fieldIds_/fieldTypes_ existants) :
...
Le problème : dans `load()`, on a `ClientContext*` et le catalog. Mais dans le constructeur normal (appelé depuis `create_tantivy_index.cpp`), on n'a pas le contexte. Il faut soit :
- Passer les types en paramètre du constructeur
- Résoudre dans `load()` uniquement (à la création, `create_tantivy_index.cpp` passe les types)

**Constructeur modifié :**
```cpp
TantivyIndex(IndexInfo indexInfo, std::unique_ptr<IndexStorageInfo> storageInfo,
    rust::Box<::TantivyHandle> handle, std::vector<PhysicalTypeID> physicalTypes = {});
```

`create_tantivy_index.cpp` passe les types collectés. `load()` les résout depuis le catalog.
...
### Option C : Encoder le type original dans le schema JSON Tantivy (~10 lignes)

**Fichier :** `create_tantivy_index.cpp` + `tantivy_index.cpp`

Stocker un hint dans le schema JSON envoyé à Tantivy :
```json
{"name":"published","type":"i64","stored":true,"indexed":true,"fast":true,"kuzu_type":"bool"}
```

Puis dans `TantivyIndex` constructeur, quand on lit les field infos depuis `get_field_ids()`, on parse aussi le `kuzu_type` hint.
...
- OU stocker le schema JSON dans TantivyIndexAuxInfo (déjà fait !) et le parser dans le constructeur C++

Le `TantivyIndexAuxInfo` stocke déjà `schemaJson`. On peut le parser dans `load()` pour extraire les `kuzu_type` hints.
...
**Ce qui a ete fait :**
- `create_tantivy_index.cpp` : `mapLogicalTypeToTantivy()` gere BOOL et TIMESTAMP → "i64", `originalTypeID` pour bulk indexing
- `tantivy_index.cpp` : `indexInfo.keyDataTypes[f] == PhysicalTypeID::BOOL` → `getValue<bool>() ? 1 : 0` dans insert() et finalize()
- Test E2E `TantivyBoolTimestampFilterTest` : 5 sous-tests — PASSED
