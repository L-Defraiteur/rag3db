...
Le projet intègre un moteur de recherche full-text (Tantivy) dans **rag3db** (fork de Kuzu), pour servir de backend à **Rag3Weaver** (framework RAG TypeScript).
...
### Phases 1-8 (1er-13 février) — Moteur Tantivy

Détails dans `docs/13-fevrier-2026-18h57/01-etat-des-lieux.md`.

- **Crate Rust FFI** `tantivy_fts` — bridge cxx typé (9 structs, 15 fonctions)
...
- **Tests** : 1015 ld-tantivy = tout vert

### Phase A (13-14 février) — Extension C++ `tantivy_fts`
...
CALL QUERY_TANTIVY_INDEX('doc', '{"type":"contains","field":"body","value":"test"}', 10,
  allowed_ids := [CAST(0, 'UINT64'), CAST(2, 'UINT64')])
RETURN node_id, score, highlights;

-- Rechercher avec filtres natifs sur colonnes non-texte
CALL QUERY_TANTIVY_INDEX('doc', '{"type":"contains","field":"body","value":"programming",
...
**Architecture** :
- `TantivyIndex` hérite de `storage::Index` — hooks automatiques (`insert`/`delete_`/`checkpoint`)
...
**Fichiers créés** (dans `extension/tantivy_fts/`) :
```
src/include/index/tantivy_index.h
src/include/catalog/tantivy_index_catalog_entry.h
src/include/function/create_tantivy_index.h
src/include/function/query_tantivy_index.h
src/include/function/drop_tantivy_index.h
src/index/tantivy_index.cpp
src/catalog/tantivy_index_catalog_entry.cpp
src/function/create_tantivy_index.cpp
src/function/query_tantivy_index.cpp
src/function/drop_tantivy_index.cpp
```
...
1. **TantivyCreateAndQueryTest** — 6 types de query (contains, term, fuzzy, phrase, parse, c++)
2. **TantivyDropTest** — CREATE → query OK → DROP → query échoue
3. **TantivyErrorTest** — erreurs (pas d'index, propriété inexistante, type non-STRING)
4. **TantivyStemmerTest** — stemmer français
5. **TantivyFilteredSearchTest** — filtrage par node IDs (`allowed_ids`)
6. **TantivyFilterFieldsTest** — filter fields natifs (INT64, DOUBLE) avec opérateurs (eq, gt, gte, multi-filtres)
7. **TantivyPersistenceTest** — create → close DB → reopen → query = mêmes résultats
...
Valide : sérialisation `IndexStorageInfo` → désérialisation → `TantivyIndex::load()` → `open_index()` → `finalize()`.
...
Câblage de `search_filtered_with_highlights` (déjà disponible côté Rust) dans `QUERY_TANTIVY_INDEX` via paramètre optionnel Cypher.

**Syntaxe** :
```cypher
CALL QUERY_TANTIVY_INDEX('doc', '{"type":"contains",...}', 10,
  allowed_ids := [CAST(0, 'UINT64'), CAST(2, 'UINT64')])
RETURN node_id, score, highlights;
```

**Changement** : `query_tantivy_index.cpp` — extraction du paramètre optionnel `allowed_ids` via `NestedVal`, dispatch vers `search_filtered_with_highlights` quand présent.

Utile pour le pré-filtrage Cypher : `MATCH ... WHERE ... WITH collect(id) AS ids CALL QUERY_TANTIVY_INDEX(...)`.

### Filter fields natifs v2 (14 février) — NOUVEAU

Colonnes non-texte (INT64, DOUBLE, UINT64, FLOAT, STRING exacte) indexées directement dans Tantivy pour filtrage **pendant** le scoring — pas de pré-filtrage par IDs nécessaire.

#### Côté C++ (create_tantivy_index.cpp)

- `FilterFieldInfo` struct (name, propertyID, tantivyType)
- `mapLogicalTypeToTantivy()` — INT64/INT32/INT16/INT8 → "i64", UINT64/32/16/8 → "u64", DOUBLE/FLOAT → "f64", STRING → "string"
...
#### Côté C++ (tantivy_index.cpp)

- `fieldTypes_` vector + `hasMixedFields_` flag dans `TantivyIndex`
...
|-------|----------|----------|
| ld-tantivy lib | 1015 pass | `cargo test --lib` |
| Extension E2E (GTest) | **7 pass** (fichier + in-memory) | `./tantivy_fts_test` |
...
| QUERY_TANTIVY_INDEX (6 types de query + highlights) | OK |
...
- **DELETE support** dans `TantivyIndex::delete_()` (implémenté mais non testé E2E)
...
│       ├── src/                       ← Moteur Tantivy modifié (1015 tests)
│       └── tantivy_fts/rust/src/
│           ├── bridge.rs              ← cxx bridge (9 structs, 15 fonctions)
│           ├── handle.rs              ← TantivyHandle, tri-field layout, f64 support
│           └── query.rs               ← build_query, FilterClause, build_filter_clause
├── extension/tantivy_fts/
│   ├── src/
│   │   ├── main/tantivy_fts_extension.cpp  ← Point d'entrée extension
│   │   ├── index/tantivy_index.cpp         ← TantivyIndex + mixed fields support
│   │   ├── catalog/                        ← TantivyIndexAuxInfo
│   │   └── function/
│   │       ├── create_tantivy_index.cpp    ← filter_fields + add_document_mixed
│   │       ├── query_tantivy_index.cpp     ← allowed_ids + highlights
│   │       └── drop_tantivy_index.cpp      ← Standalone + Internal
│   ├── test/
│   │   └── tantivy_fts_test.cpp            ← 7 tests GTest E2E
...
IN_MEM_MODE=true LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu ./extension/tantivy_fts/test/tantivy_fts_test
```
