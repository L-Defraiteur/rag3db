...
Fichier : `extension/lucivy/ld-lucivy/lucivy_fts/rust/src/handle.rs`
...
Fichier : `extension/lucivy/ld-lucivy/src/query/phrase_query/ngram_contains_query.rs`
...
**Probleme de build, pas de code.** Le `CMakeLists.txt` de lucivy_fts utilise `add_custom_command(OUTPUT liblucivy_fts.a ...)` sans `DEPENDS` sur les sources Rust. Consequence :
...
Le piege a ete documente dans `extension/lucivy_fts/BUILD.md` et `BUILD.md` a la racine.

## Tests passes (tout vert)

- **Tests unitaires Rust** : 1015 passed
- **10 tests GTest E2E** : tous passent (y compris le nouveau `LucivyNgramContainsNoStemmerTest`)
...
|------|--------|--------|---------|
| ld-lucivy | `main` | `4c4e7ad` | handle.rs + ngram_contains_query.rs + README rebuild docs |
...
| `extension/lucivy_fts/BUILD.md` | Architecture 3 couches, piege cmake/cargo, commande tout-en-un |
| `ld-lucivy/README.md` | Restructure : NgramContainsQuery en avant, triple-field layout, BM25, exemples de matchs |
