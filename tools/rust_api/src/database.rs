...
/// The Database class is the main class of `Rag3dbDB`. It manages all database components.
pub struct Database {
...
pub struct SystemConfig {
    ...
    /// If true, any WAL replaying failure when loading the database will throw an error. Otherwise, Rag3db will silently ignore the failure and replay up to where the error occured.
...
mod tests {
    ...
    fn test_database_in_memory() -> Result<()> {
        ...
        // If the special name is ever changed (or removed) rag3db is likely to just create a db directory with that name
