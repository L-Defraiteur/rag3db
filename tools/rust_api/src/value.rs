...
pub enum ConversionError {
    /// Rag3db's internal date as the number of days since 1970-01-01
    Date(i32),
    /// Rag3db's internal timestamp as the number of microseconds since 1970-01-01
    Timestamp(i64),
    TimestampTz(i64),
    TimestampNs(i64),
    TimestampMs(i64),
...
impl std::fmt::Debug for ConversionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            Date(days) => write!(f, "Could not convert Rag3db date offset of UNIX_EPOCH + {days} days to time::Date"),
            Timestamp(us) => write!(f, "Could not convert Rag3db timestamp offset of UNIX_EPOCH + {us} microseconds to time::OffsetDateTime"),
            TimestampTz(us) => write!(f, "Could not convert Rag3db timestamp_tz offset of UNIX_EPOCH + {us} microseconds to time::OffsetDateTime"),
            TimestampNs(ns) => write!(f, "Could not convert Rag3db timestamp_ns offset of UNIX_EPOCH + {ns} nanoseconds to time::OffsetDateTime"),
            TimestampMs(ms) => write!(f, "Could not convert Rag3db timestamp_ms offset of UNIX_EPOCH + {ms} milliseconds to time::OffsetDateTime"),
            TimestampSec(sec) => write!(f, "Could not convert Rag3db timestamp_sec offset of UNIX_EPOCH + {sec} seconds to time::OffsetDateTime"),
        }
    }
}
...
/// Data types supported by Rag3db
///
/// Also see <https://rag3db.com/docusaurus/cypher/data-types/overview.html>
#[derive(Clone, Debug, PartialEq)]
pub enum Value {
    Null(LogicalType),
    Bool(bool),
    ...
    ///
    /// <https://rag3db.com/docusaurus/cypher/data-types/date.html>
    Date(time::Date),
    ...
    ///
    /// <https://rag3db.com/docusaurus/cypher/data-types/interval.html>
    ...
    ///
    /// <https://rag3db.com/docusaurus/cypher/data-types/timestamp.html>
    ...
    /// <https://rag3db.com/docusaurus/cypher/data-types/string.html>
    String(String),
    Blob(Vec<u8>),
    ...
    /// <https://rag3db.com/docusaurus/cypher/data-types/list.html>
    List(LogicalType, Vec<Value>),
    /// These must contain elements which are all the same type.
    /// <https://rag3db.com/docusaurus/cypher/data-types/list.html>
    Array(LogicalType, Vec<Value>),
    /// <https://rag3db.com/docusaurus/cypher/data-types/struct.html>
    ...
    /// <https://rag3db.com/docusaurus/cypher/data-types/map>
    Map((LogicalType, LogicalType), Vec<(Value, Value)>),
    /// <https://rag3db.com/docusaurus/cypher/data-types/union>
    Union {
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            // Note: These don't match rag3db's toString, but we probably don't want them to
    ...
    fn try_from(value: &ffi::Value) -> Result<Self, Self::Error> {
        ...
        match ffi::value_get_data_type_id(value) {
            ...
            LogicalTypeID::INTERVAL => Ok(Value::Interval(time::Duration::new(
                ...
                // Duration is constructed using nanoseconds, but rag3db stores microseconds
...
impl TryInto<cxx::UniquePtr<ffi::Value>> for Value {
    ...
    fn try_into(self) -> Result<cxx::UniquePtr<ffi::Value>, Self::Error> {
        ...
        fn date_to_rag3db_date_t(value: time::Date) -> i32 {
            // Convert to days since 1970-01-01
            (value - time::Date::from_ordinal_date(1970, 1).unwrap())
                .whole_days()
                .to_i32()
                .expect("rag3db_date i64->i32 overflow")
        }
        ...
        match self {
            ...
            Value::Date(value) => Ok(ffi::create_value_date(date_to_rag3db_date_t(value))),
...
mod tests {
    ...
    macro_rules! type_tests {
        ...
        $(
            #[test]
            /// Tests that the values are correctly converted into `rag3db::common::Value` and back
            fn $name() -> Result<()> {
    ...
    database_tests! {
        // Passing these values as arguments is not yet implemented in rag3db:
    ...
    #[test]
    /// Test that the timestamp round-trips through rag3db's internal timestamp
