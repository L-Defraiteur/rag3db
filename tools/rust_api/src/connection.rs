...
///
/// ```
/// # use rag3db::{Connection, Database, SystemConfig, Value, Error};
...
impl<'a> Connection<'a> {
    ...
    ///
    /// # Arguments
    /// * `query`: The query to prepare. See <https://rag3db.com/docs/cypher> for details on the
    ///   query format.
    ...
    ///
    /// # Arguments
    /// * `query`: The query to execute. See <https://rag3db.com/docs/cypher> for details on the
    ///   query format.
    // TODO(bmwinger): Instead of having a Value enum in the results, perhaps QueryResult, and thus query
    // should be generic.
    //
    // E.g.
    // let result: QueryResult<rag3db::value::List<rag3db::value::String>> = conn.query("...")?;
    // let result: QueryResult<rag3db::value::Int64> = conn.query("...")?;
    //
    ...
    ///```
    /// # use rag3db::{Database, SystemConfig, Connection, Value};
    /// # use anyhow::Error;
    /// #
        ...
        $(
            ...
            fn $name() -> Result<()> {
                ...
                let directory: String = if cfg!(windows) {
                    std::env::var("RAG3DB_LOCAL_EXTENSIONS")?.replace("\\", "/")
                } else {
                    std::env::var("RAG3DB_LOCAL_EXTENSIONS")?
                };
                let name = stringify!($name);
                conn.query(&format!("LOAD EXTENSION '{directory}/{name}/build/lib{name}.rag3db_extension'"))?;
                Ok(())
            }
        )*
        }
