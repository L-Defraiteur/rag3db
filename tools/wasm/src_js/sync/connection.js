...
class Connection {
   ...
   *
   * @param {rag3db.sync.Database} database the database object to connect to.
  ...
  constructor(database, numThreads = null) {
    ...
    const rag3db = Rag3dbWasm._rag3db;
    ...
    this._connection = new rag3db.Connection(database._database);
   ...
   * @returns {rag3db.sync.QueryResult} the query result.
   * @throws {Error} if the connection is closed.
   ...
   * @returns {Promise<rag3db.sync.PreparedStatement>} the prepared statement.
   */
  prepare(statement) {
    this._checkConnection();
    if (typeof statement !== "string") {
   ...
   * @param {rag3db.sync.PreparedStatement} preparedStatement the prepared 
