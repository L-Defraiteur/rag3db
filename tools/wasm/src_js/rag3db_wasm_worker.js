/**
 * @file rag3db_wasm_worker.js is the file for the worker thread that wraps around 
...
if (!isWorkerRuntime) {
  ...
  expose({
    ...
    getVersion() {
      return rag3dbSync.getVersion();
    },

    getStorageVersion() {
      return rag3dbSync.getStorageVersion();
    },
    ...
    databaseConstruct(databasePath,
      ...
      const id = uuidv4();
      try {
        objectsStore[id] = new rag3dbSync.Database(
          databasePath,
          bufferPoolSize,
          maxNumThreads,
          enableCompression,
    ...
    connectionConstruct(databaseId, numThreads = null) {
      ...
      const id = uuidv4();
      try {
        objectsStore[id] = new rag3dbSync.Connection(
