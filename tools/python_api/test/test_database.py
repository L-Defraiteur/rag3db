...
def open_database_on_subprocess(tmp_path: Path, build_dir: Path) -> None:
    code = dedent(
        ...
        import rag3db
        db = rag3db.Database(r"{tmp_path!s}")
        print(r"{tmp_path!s}")
    """
    )
...
def test_database_close(tmp_path: Path, build_dir: Path) -> None:
    db_path = tmp_path / "test_database_close.rag3db"
    db = rag3db.Database(database_path=db_path, read_only=False)
...
def test_database_context_manager(tmp_path: Path, build_dir: Path) -> None:
    db_path = tmp_path / "test_database_context_manager.rag3db"
    with rag3db.Database(database_path=db_path, read_only=False) as db:
        assert not db.is_closed
        assert db._database is not None
...
def test_in_mem_database_memory_db_path() -> None:
    db = rag3db.Database(database_path=":memory:")
    assert not db.is_closed
    assert db._database is not None
...
def test_in_mem_database_no_db_path() -> None:
    with rag3db.Database(database_path="") as db:
        assert not db.is_closed
        assert db._database is not None
...
def test_database_auto_checkpoint_config(tmp_path: Path) -> None:
    with rag3db.Database(database_path=get_db_file_path(tmp_path), auto_checkpoint=False) as db:
        assert not db.is_closed
        assert db._database is not None
...
def test_database_checkpoint_threshold_config(tmp_path: Path) -> None:
    with rag3db.Database(database_path=get_db_file_path(tmp_path), checkpoint_threshold=1234) as db:
        assert not db.is_closed
        assert db._database is not None
...
def test_database_throw_on_wal_replay_failure_config(tmp_path: Path) -> None:
    ...
    with Path.open(wal_file_path, "w") as wal_file:
        wal_file.write("a" * 28)
    with rag3db.Database(database_path=database_path, throw_on_wal_replay_failure=False) as db:
        assert not db.is_closed
        assert db._database is not None
...
def test_database_enable_checksums_config(tmp_path: Path) -> None:
    ...
    with rag3db.Database(database_path=database_path) as db:
        assert not db.is_closed
        assert db._database is not None
        conn = rag3db.Connection(db)
    ...
    with pytest.raises(RuntimeError) as check:
        db = rag3db.Database(database_path=database_path, enable_checksums=False)


def test_database_close_order() -> None:
    in_mem_db = rag3db.Database(database_path=":memory:", buffer_pool_size=1024 * 1024 * 10)
    assert not in_mem_db.is_closed
    assert in_mem_db._database is not None

    in_mem_conn = rag3db.Connection(in_mem_db)
