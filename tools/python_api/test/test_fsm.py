...
def fsm_node_table_setup(tmp_path: Path):
    db = rag3db.Database(get_db_file_path(tmp_path))
    ...
    conn.execute(
        f"COPY person FROM ['{RAG3DB_ROOT}/dataset/tinysnb/vPerson.csv', '{RAG3DB_ROOT}/dataset/tinysnb/vPerson2.csv'](ignore_errors=true, header=false)"
    )
    return db, conn
...
def fsm_rel_table_setup(fsm_node_table_setup):
    ...
    conn.execute(
        f"COPY knows FROM ['{RAG3DB_ROOT}/dataset/tinysnb/eKnows.csv', '{RAG3DB_ROOT}/dataset/tinysnb/eKnows_2.csv']"
    )
...
def fsm_rel_group_setup(tmp_path: Path):
    ...
    conn.execute(f'COPY personA FROM "{RAG3DB_ROOT}/dataset/rel-group/node.csv";')
    conn.execute(f'COPY personB FROM "{RAG3DB_ROOT}/dataset/rel-group/node.csv";')
    conn.execute(f'COPY likes FROM "{RAG3DB_ROOT}/dataset/rel-group/edge.csv" (FROM="personA", TO="personB");')
    conn.execute(f'COPY likes FROM "{RAG3DB_ROOT}/dataset/rel-group/edge.csv" (FROM="personB", TO="personA");')
    return db, conn
