...
int main(int argc, char* argv[]) {
    args::ArgumentParser parser("Rag3db shell");
    ...
    if (version) {
        std::cout << "Rag3db " << RAG3DB_CMAKE_VERSION << '\n';
        return 0;
    }
    ...
    if (pathToHistory.empty() && !std::filesystem::exists(historyFile)) {
        ...
        if (!homeDir.empty()) {
            pathToHistory = std::string(homeDir) + "/.rag3db/";
