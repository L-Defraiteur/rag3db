...
def set_up_search(test) -> None:
    ...
    test.send_finished_statement('RETURN "rag3db is cool" AS b;\r')
    assert (
        test.shell_process.expect_exact(["\u2502 rag3db is cool \u2502", pexpect.EOF])
        == 0
    )
...
def test_ctrl_u_cleared(test) -> None:
    ...
    test.send_finished_statement('RETURN "rag3db is cool" AS a;\r')
    assert (
        test.shell_process.expect_exact(["\u2502 rag3db is cool \u2502", pexpect.EOF])
        == 0
    )
...
def test_ctrl_d_eof(test) -> None:
    ...
    assert test.shell_process.expect_exact(["rag3db", pexpect.EOF]) == 0
    # acts as EOF
    assert test.shell_process.expect_exact(["rag3db", pexpect.EOF]) == 1
