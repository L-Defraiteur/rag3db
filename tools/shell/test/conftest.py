...
from test_helper import RAG3DB_EXEC_PATH, RAG3DB_ROOT, deleteIfExists
...
class ShellTest:
    ...
    def send_finished_statement(self, stmt: str) -> None:
        if self.shell_process:
            assert self.shell_process.expect_exact(["rag3db", pexpect.EOF]) == 0
...
@pytest.fixture()
def history_path():
    path = os.path.join(RAG3DB_ROOT, "tools", "shell", "test", "files")
...
@pytest.fixture()
def csv_path():
    return os.path.join(RAG3DB_ROOT, "tools", "shell", "test", "files", "vPerson.csv")


@pytest.fixture()
def init_path():
    return os.path.join(RAG3DB_ROOT, "tools", "shell", "test", "files", "start.cypher")
