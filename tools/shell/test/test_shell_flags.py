...
from test_helper import RAG3DB_VERSION, deleteIfExists
...
def test_help(temp_db, flag) -> None:
    ...
    result = test.run()
    result.check_stdout("Rag3db shell")
    # with database path
...
def test_default_bp_size(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(temp_db).add_argument(flag).add_argument("rag3db")
    result = test.run()
    result.check_stderr("Argument '' received invalid value type 'rag3db'")
...
def test_max_db_size(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(temp_db).add_argument(flag).add_argument("rag3db")
    result = test.run()
    result.check_stderr("Argument 'max_db_size' received invalid value type 'rag3db'")
...
def test_read_only(temp_db, flag) -> None:
    ...
    test = (
        ...
        .statement('RETURN "rag3db is cool" AS b;')
    )
    result = test.run()
    ...
    result.check_stdout("rag3db is cool")
...
def test_history_path(temp_db, history_path) -> None:
    ...
    with open(os.path.join(history_path, "history.txt")) as f:
        assert f.readline() == 'RETURN "databases rule" AS a;\n'
        assert f.readline() == 'RETURN "rag3db is cool" AS b;\n'
...
def test_version(temp_db, flag) -> None:
    ...
    test = ShellTest().add_argument(flag)
    result = test.run()
    result.check_stdout(RAG3DB_VERSION)
    # with database path
    test = ShellTest().add_argument(temp_db).add_argument(flag)
    result = test.run()
    result.check_stdout(RAG3DB_VERSION)
...
def test_mode(temp_db, flag) -> None:
    ...
    test = (
        ShellTest()
        .add_argument(temp_db)
        .statement('RETURN "Databases Rule" AS a, "rag3db is cool" AS b;')
    )
    result = test.run()
    result.check_stdout("\u2502 a              \u2502 b            \u2502")
    result.check_stdout("\u2502 Databases Rule \u2502 rag3db is cool \u2502")
    ...
    result.check_stdout("</tr>")
    result.check_stdout("<tr>")
    result.check_stdout("<td>Databases Rule</td><td>rag3db is cool</td>")
    result.check_stdout("</tr>")
    result.check_stdout("</table>")
    ...
    result = test.run()
    result.check_stdout('[\n{"a":"Databases Rule","b":"rag3db is cool"}\n]')
    ...
    result = test.run()
    result.check_stdout('{"a":"Databases Rule","b":"rag3db is cool"}')
    ...
    result.check_stdout("b = rag3db is cool")
