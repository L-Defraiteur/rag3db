...
public class ValueTest extends TestBase {
    ...
    void CreateListLiteral() {
        ...
        Rag3dbList list = new Rag3dbList(listValues);
    ...
    void CreateListLiteralNested() {
        ...
        Rag3dbList[] nestedLists = { new Rag3dbList(nestedListValues[0]), new Rag3dbList(nestedListValues[1]) };

        Value[] listValues = { nestedLists[0].getValue(), nestedLists[1].getValue() };
        Rag3dbList list = new Rag3dbList(listValues);
        assertEquals(listValues.length, list.getListSize());
        for (int i = 0; i < list.getListSize(); ++i) {
            Rag3dbList nestedList = new Rag3dbList(list.getListElement(i));
    ...
    void CreateListDefaultValues() {
        int listLength = 5;
        Rag3dbList list = new Rag3dbList(new DataType(DataTypeID.INT32), listLength);
    ...
    void ValueGetListSize() {
        ...
        assertFalse(value.isNull());
        Rag3dbList list = new Rag3dbList(value);
    ...
    void CreateStructLiteral() {
        ...
        Rag3dbStruct structVal = new Rag3dbStruct(fieldNames, fieldValues);
    ...
    void CreateStructLiteralNested() {
        ...
        Rag3dbStruct person = new Rag3dbStruct(personFieldNames, personFieldValues);

        String[] companyFieldNames = { "name", "boss" };
        Value[] companyFieldValues = { new Value("Company"), person.getValue() };
        Rag3dbStruct company = new Rag3dbStruct(companyFieldNames, companyFieldValues);

        Rag3dbStruct actualPerson = new Rag3dbStruct(company.getValueByFieldName("boss"));
    ...
    void CreateStructLiteralFromMap() {
        Map<String, Value> fields = Map.of(
                ...
                "ID", new Value(2),
                "age", new Value(20));
        Rag3dbStruct structVal = new Rag3dbStruct(fields);
    ...
    void StructValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        Rag3dbStruct structVal = new Rag3dbStruct(value);
    ...
    void CreateMapLiteral() {
        ...
        Rag3dbMap rag3dbMap = new Rag3dbMap(keys, values);
        assertEquals(keys.length, rag3dbMap.getNumFields());
        int aliceKeyIdx = (rag3dbMap.getKey(0).getValue().equals("Alice")) ? 0 : 1;
        int bobKeyIdx = 1 - aliceKeyIdx;
        assertEquals("Alice", rag3dbMap.getKey(aliceKeyIdx).getValue());
        assertEquals(1, (Integer) rag3dbMap.getValue(aliceKeyIdx).getValue());
        assertEquals("Bob", rag3dbMap.getKey(bobKeyIdx).getValue());
        assertEquals(2, (Integer) rag3dbMap.getValue(bobKeyIdx).getValue());

        PreparedStatement stmt = conn
                .prepare("MATCH (m:movies) WHERE m.name = 'Roma' SET m.audience = $audience");
        Map<String, Value> options = Map.of(
                "audience", rag3dbMap.getValue());
    ...
    void CreateMapLiteralNested() {
        ...
        Rag3dbMap map0 = new Rag3dbMap(nestedKeys[0], nestedValues[0]);
        Rag3dbMap map1 = new Rag3dbMap(nestedKeys[1], nestedValues[1]);
        Value[] nestedMaps = { map0.getValue(), map1.getValue() };

        Value[] keys = { new Value(Long.valueOf(0)), new Value(Long.valueOf(1)) };
        Rag3dbMap rag3dbMap = new Rag3dbMap(keys, nestedMaps);
        assertEquals(keys.length, rag3dbMap.getNumFields());

        Rag3dbMap map1Actual = new Rag3dbMap(rag3dbMap.getValue(1));
    ...
    void MapValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        Rag3dbMap mapVal = new Rag3dbMap(value);
        ...
        try (QueryResult result = conn
            ...
            try (FlatTuple flatTuple = result.getNext();
                ...
                try (Value nodeListValue = ValueRecursiveRelUtil.getNodeList(value)) {
                    ...
                    Rag3dbList nodeList = new Rag3dbList(nodeListValue);
                ...
                try (Value relListValue = ValueRecursiveRelUtil.getRelList(value)) {
                    ...
                    Rag3dbList relList = new Rag3dbList(relListValue);
