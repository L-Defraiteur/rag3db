...
public class Rag3dbMap implements AutoCloseable {
     ...
     */
    public Rag3dbMap(Value[] keys, Value[] values) {
        ...
        mapVal = Native.rag3dbCreateMap(keys, values);
    }
    ...
    private Value getMapKeyOrValue(long index, boolean isKey) {
        ...
        Value structValue = Native.rag3dbValueGetListElement(mapVal, index);
        Value keyOrValue = new Rag3dbList(structValue).getListElement(isKey ? 0 : 1);
        structValue.close();
        return keyOrValue;
    }
    ...
    public long getNumFields() {
        ...
        return Native.rag3dbValueGetListSize(mapVal);
    }
