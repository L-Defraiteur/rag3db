...
def export_datasets_and_test(
    ...
    if not os.path.exists(export_path + os.sep):
        ...
        export_script_path = os.path.join(rag3db_root, "scripts", "export-dbs.py")
        exec_path = os.path.join(
            base_worktree, "build", "relwithdebinfo", "tools", "shell", "rag3db"
        )
        run_command(
...
def write_split_testfile(
    ...
    def replace_placeholders(lines):
        return [
            line.replace("${RAG3DB_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, ""))
            for line in lines
        ]
    ...
    with open(export_path, "w") as f:
        f.write(header.replace("${RAG3DB_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, "")))
        f.writelines(replace_placeholders(export_lines))

    with open(import_path, "w") as f:
        f.write(
            transform_import_header(
                header.replace("${RAG3DB_EXPORT_DB_DIRECTORY}", os.path.join(db_dir, ""))
            )
        )
...
def run_export_specific_tests(
    rag3db_root, base_worktree, test_worktree, test_dir, output_dir, cleanup
):
    # Split tests in test_dir
    split_files(test_dir, output_dir)
    # Build base_worktree rag3db
...
def main():
    ...
    try:
        ...
        rag3db_root = os.path.abspath(os.path.join(script_dir, ".."))
        base_worktree = os.path.join(rag3db_root, ".worktree-base")
        test_worktree = os.path.join(rag3db_root, ".worktree-test")
        create_worktree(base_worktree, base_commit, rag3db_root)
        create_worktree(test_worktree, test_commit, rag3db_root)
        ...
        if base_worktree or test_worktree:
            ...
            if base_worktree:
                remove_worktree(base_worktree, rag3db_root)
            if test_worktree:
                remove_worktree(test_worktree, rag3db_root)
