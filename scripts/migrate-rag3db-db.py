#!/usr/bin/env python3
"""
Rag3db Database Migration Script

This script helps migrate Rag3db databases between versions.
- Sets up isolated Python environments for each Rag3db version
...
Usage Examples:
    ...
    python migrate-rag3db-db.py --old-version 0.9.0 --new-version 0.11.0 --old-db /path/to/old/database --new-db /path/to/new/database

Notes:
- Can only be used to migrate to newer Rag3db versions, from 0.11.0 onwards
"""
...
RAG3DB_FILE_EXTENSIONS = ["", ".wal", ".shadow"]


# FIXME: Replace this with a Rag3db query to get the mapping when available.
rag3db_version_mapping = {
    34: "0.7.0",
    35: "0.7.1",
    36: "0.8.2",
    37: "0.9.0",
...
minimum_rag3db_migration_version = "0.11.0"


def rag3db_version_comparison(version: str, target: str) -> bool:
    """Return True if Rag3db *v* is greater or equal to target version"""
...
def read_rag3db_storage_version(rag3db_db_path: str) -> int:
    """
    Reads the Rag3db storage version.

    :param rag3db_db_path: Path to the Rag3db database file/directory.
    :return: Storage version code as an integer.
    """
    if os.path.isdir(rag3db_db_path):
        rag3db_version_file_path = os.path.join(rag3db_db_path, "catalog.kz")
        if not os.path.isfile(rag3db_version_file_path):
            raise FileNotFoundError("Rag3db catalog.kz file does not exist")
    else:
        rag3db_version_file_path = rag3db_db_path

    with open(rag3db_version_file_path, "rb") as f:
        f.seek(4)
        # Read the next 8 bytes as a little-endian unsigned 64-bit integer
        data = f.read(8)
        if len(data) < 8:
            raise ValueError(
                f"File '{rag3db_version_file_path}' does not contain a storage version code."
            )
        version_code = struct.unpack("<Q", data)[0]

    if version_code in rag3db_version_mapping:
        return rag3db_version_mapping[version_code]
    else:
        raise ValueError(f"Could not map version_code {version_code} to proper Rag3db version.")


def ensure_env(version: str, export_dir) -> str:
    """
    Creates a venv at `{export_dir}/.rag3db_envs/{version}` and installs `rag3db=={version}`
    Returns the venv's python executable path.
    """
    # Use temp directory to create venv
    rag3db_envs_dir = os.path.join(export_dir, ".rag3db_envs")

    # venv base under the script directory
    base = os.path.join(rag3db_envs_dir, version)
    py_bin = os.path.join(base, "bin", "python")
    # If environment already exists clean it
    if os.path.isdir(base):
        shutil.rmtree(base)

    print(f"→ Setting up venv for Rag3db {version}...", file=sys.stderr)
    # Create venv
    ...
    # Install the specific Rag3db version
    subprocess.run([py_bin, "-m", "pip", "install", "--upgrade", "pip"], check=True)
    subprocess.run([py_bin, "-m", "pip", "install", f"rag3db=={version}"], check=True)
    return py_bin


def run_migration_step(python_exe: str, db_path: str, cypher: str):
    """
    Uses `python_exe` to connect to the Rag3db database at `db_path` and run the `cypher` query.
    """
    snippet = f"""
import rag3db
db = rag3db.Database(r"{db_path}")
...
def rag3db_migration(
    ...
    if not rag3db_version_comparison(
        version=new_version, target=minimum_rag3db_migration_version
    ):
        raise ValueError(
            f"New version for rag3db is not supported, has to be equal or higher than version: {minimum_rag3db_migration_version}"
        )

    print(
        f"Migrating Rag3db database from {old_version} to {new_version}", file=sys.stderr
    )
    print(f"Source: {old_db}", file=sys.stderr)
    print("", file=sys.stderr)

    # If version of old rag3db db is not provided try to determine it based on file info
    if not old_version:
        old_version = read_rag3db_storage_version(old_db)
    ...
    with tempfile.TemporaryDirectory() as export_dir:
        # Set up environments
        print(f"Setting up Rag3db {old_version} environment...", file=sys.stderr)
        old_py = ensure_env(old_version, export_dir)
        print(f"Setting up Rag3db {new_version} environment...", file=sys.stderr)
        new_py = ensure_env(new_version, export_dir)

        export_file = os.path.join(export_dir, "rag3db_export")
    ...
    # Rename new rag3db database to old rag3db database name if enabled
    if overwrite or delete_old:
        # Remove rag3db lock from migrated DB
        lock_file = new_db + ".lock"
        if os.path.exists(lock_file):
            os.remove(lock_file)
        rename_databases(old_db, old_version, new_db, delete_old)

    print("Rag3db graph database migration finished successfully!")
...
def rename_databases(old_db: str, old_version: str, new_db: str, delete_old: bool):
    ...
    if os.path.isfile(old_db):
        ...
        for ext in RAG3DB_FILE_EXTENSIONS:
            src = old_db + ext
            dst = backup_base + ext
        ...
        # Directory-based Rag3db database
...
def main():
    p = argparse.ArgumentParser(
        description="Migrate Rag3db DB via PyPI versions",
        epilog="""
Examples:
  %(prog)s --old-version 0.9.0 --new-version 0.11.0 \\
    --old-db /path/to/old/db --new-db /path/to/new/db --overwrite

Note: This script will create temporary virtual environments in .rag3db_envs/ directory
to isolate different Rag3db versions.
        """,
    ...
    p.add_argument(
        ...
        required=False,
        default=None,
        help="Source Rag3db version (e.g., 0.9.0). If not provided, automatic rag3db version detection will be attempted.",
    )
    p.add_argument(
        "--new-version", required=True, help="Target Rag3db version (e.g., 0.11.0)"
    )
