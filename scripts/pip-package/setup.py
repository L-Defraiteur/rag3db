...
with open(os.path.join(base_dir, 'rag3db-source', 'tools', 'python_api', 'requirements_dev.txt')) as f:
    requirements = f.read().splitlines()


def _get_rag3db_version():
    cmake_file = os.path.join(base_dir, 'rag3db-source', 'CMakeLists.txt')
...
print("The version of this build is %s" % rag3db_version)
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        build_dir = os.path.join(ext.sourcedir, 'rag3db-source')
...
setup(name='rag3db',
      version=rag3db_version,
      install_requires=[],
      ext_modules=[CMakeExtension(
          name="rag3db", sourcedir=base_dir)],
