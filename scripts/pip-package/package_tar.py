...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        file_name = "rag3db-%s.tar.gz" % _get_rag3db_version()
    ...
    with TemporaryDirectory() as tempdir:
        subprocess.check_call(
            [
                ...
                "tar",
                "-o",
                os.path.join(tempdir, "rag3db-source.tar"),
                "HEAD",
            ],
            cwd="../..",
        )

        with tarfile.open(os.path.join(tempdir, "rag3db-source.tar")) as tar:
            tar.extractall(path=os.path.join(tempdir, "rag3db-source"), filter=None)

        os.remove(os.path.join(tempdir, "rag3db-source.tar"))

        # Remove components that are not needed for the pip package
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/dataset"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/examples"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/benchmark"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/logo"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/extension"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/test"))
        shutil.rmtree(os.path.join(tempdir, "rag3db-source/.github"))

        os.makedirs(os.path.join(tempdir, "rag3db"))
        ...
        shutil.copy2(
            os.path.join(tempdir, "rag3db.egg-info", "PKG-INFO"),
