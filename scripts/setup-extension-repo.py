...
def extract_extension_version() -> str:
    import re
    pattern = r'-DRAG3DB_EXTENSION_VERSION=(["\'])(.*?)\1'
    with open('./CMakeLists.txt', 'r') as f:
        for line in f:
            ...
            if match:
                ...
                return version
    raise Exception("Failed to infer RAG3DB extension version from CMAKE file.")
...
if get_os() != "win":
    ...
    if get_os() == "osx":
        ...
        file_name += '.so'
    official_repo = f"https://extension.rag3db.com/{rag3db_version}/{arch_version}/common/{file_name}"
