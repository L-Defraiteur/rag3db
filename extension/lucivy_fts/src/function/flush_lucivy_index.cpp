...
struct FlushLucivyBindData final : TableFuncBindData {
    std::string tableName;
    table_id_t tableID;
    std::string indexName;

    FlushLucivyBindData(std::string tableName, table_id_t tableID, std::string indexName)
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<FlushLucivyBindData>(*this);
    }
};
...
static std::unique_ptr<TableFuncBindData> bindFunc(ClientContext* context,
    ...
    return std::make_unique<FlushLucivyBindData>(tableName, tableEntry->getTableID(), indexName);
}

// ── rewriteFunc (public FLUSH_LUCIVY_INDEX) ─────────────────────────────────
...
static std::string rewriteFunc(ClientContext& context, const TableFuncBindData& bindData) {
    ...
    auto& bd = *bindData.constPtrCast<FlushLucivyBindData>();
    auto query = stringFormat("CALL _FLUSH_LUCIVY_INDEX('{}');", bd.tableName);
    query +=
        stringFormat("RETURN 'Lucivy index flushed for table {}.' AS result;", bd.tableName);
    return query;
}

// ── Internal _FLUSH_LUCIVY_INDEX ────────────────────────────────────────────

static offset_t internalTableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bd = *input.bindData->constPtrCast<FlushLucivyBindData>();
    ...
    if (!indexOpt.has_value()) {
        throw RuntimeException{
            stringFormat("No Lucivy index found on table '{}'.", bd.tableName)};
    }
...
function_set InternalFlushLucivyFunction::getFunctionSet() {
...
function_set FlushLucivyFunction::getFunctionSet() {
    function_set functionSet;
    auto func = std::make_unique<TableFunction>(name,
        std::vector{LogicalTypeID::STRING});
