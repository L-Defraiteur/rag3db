...
struct CreateTantivyBindData final : TableFuncBindData {
    ...
    CreateTantivyBindData(std::string tableName, table_id_t tableID, std::string indexName,
    ...
    std::unique_ptr<TableFuncBindData> copy() const override {
        return std::make_unique<CreateTantivyBindData>(*this);
    }
};
...
static std::pair<std::vector<std::string>, std::vector<property_id_t>> resolveProperties(
    ...
    for (auto i = 0u; i < propertyValue.getChildrenSize(); i++) {
        ...
        if (tableEntry->getProperty(tableEntry->getPropertyID(propName)).getType() !=
            LogicalType::STRING()) {
            throw BinderException{
                "Tantivy index can only be built on STRING properties."};
        }
...
static std::string mapLogicalTypeToTantivy(const LogicalType& type) {
...
static std::vector<FilterFieldInfo> resolveFilterFields(
    ...
    for (auto i = 0u; i < filterFieldsValue.getChildrenSize(); i++) {
        ...
        auto tantivyType = mapLogicalTypeToTantivy(propType);
        if (tantivyType.empty()) {
...
// ── Bind (public CREATE_TANTIVY_INDEX: tableName, propertyList) ──────────────
...
static std::unique_ptr<TableFuncBindData> bindFunc(ClientContext* context,
    ...
    return std::make_unique<CreateTantivyBindData>(tableName, tableEntry->getTableID(), indexName,
        std::move(propertyNames), std::move(propertyIDs), stemmer, std::move(filterFields));
}

// ── Bind (internal _CREATE_TANTIVY_INDEX: tableName, indexName, propertyList) ─
...
// ── rewriteFunc (public CREATE_TANTIVY_INDEX) ───────────────────────────────
...
static std::string rewriteFunc(ClientContext& context, const TableFuncBindData& bindData) {
    ...
    auto query = stringFormat(
        "CALL _CREATE_TANTIVY_INDEX('{}', '{}', {}, {});",
        bd.tableName, bd.indexName, properties, optionalArgs);
    query += stringFormat("RETURN 'Tantivy index created on table {}.' AS result;", bd.tableName);
    return query;
}

// ── Internal _CREATE_TANTIVY_INDEX ──────────────────────────────────────────

static offset_t tableFunc(const TableFuncInput& input, TableFuncOutput&) {
    auto& bd = *input.bindData->constPtrCast<CreateTantivyBindData>();
    ...
    auto indexPath = stringFormat("{}/tantivy_indexes/{}/", basePath, bd.tableName);
    std::filesystem::create_directories(indexPath);

    // 2. Build Tantivy schema JSON.
    ...
    // 3. Create Tantivy index via cxx bridge.
    auto handle = create_index(indexPath, schemaJson);

    // 4. Get field IDs — build mapping from field name to Tantivy field ID and type.
        ...
        for (offset_t offset = 0; offset < numRows; offset++) {
            ...
            if (!hasMixed) {
                ...
                for (size_t ff = 0; ff < bd.filterFields.size(); ff++) {
                    ...
                    } else if (ffInfo.tantivyType == "string") {
                        DocFieldText field;
                        field.field_id = fid;
    ...
    auto auxInfo = std::make_unique<TantivyIndexAuxInfo>(indexPath, schemaJson, bd.stemmer);
    auto indexEntry = std::make_unique<catalog::IndexCatalogEntry>(
        TantivyIndexCatalogEntry::TYPE_NAME, bd.tableID, bd.indexName, allPropertyIDs,
        std::move(auxInfo));
    catalog->createIndex(transaction, std::move(indexEntry));

    // 8. Create TantivyIndex and register on NodeTable.
    auto indexType = TantivyIndex::getIndexType();
    ...
    auto storageInfo = std::make_unique<TantivyStorageInfo>(numRows);
    auto tantivyIndex = std::make_unique<TantivyIndex>(
        std::move(indexInfo), std::move(storageInfo), std::move(handle));
    nodeTable.addIndex(std::move(tantivyIndex));
...
// For internal _CREATE_TANTIVY_INDEX (3 params: tableName, indexName, properties)
...
// For public CREATE_TANTIVY_INDEX (2 params: tableName, properties)
...
function_set InternalCreateTantivyFunction::getFunctionSet() {
...
function_set CreateTantivyFunction::getFunctionSet() {
    function_set functionSet;
    auto func = std::make_unique<TableFunction>(name,
