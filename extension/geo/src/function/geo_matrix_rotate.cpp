...
// GEO_MATRIX_ROTATE(matrix 9 col-major, point [x,y,z]) → LIST(DOUBLE) [x,y,z]
static void execFunc(const std::vector<std::shared_ptr<ValueVector>>& parameters,
    ...
    for (auto i = 0u; i < resultSelVector->getSelSize(); ++i) {
        ...
        double ox, oy, oz;
        mat.rotate(point[0], point[1], point[2], ox, oy, oz);
...
function_set GeoMatrixRotateFunction::getFunctionSet() {
    function_set result;
