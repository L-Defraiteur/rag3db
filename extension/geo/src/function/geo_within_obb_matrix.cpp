...
// GEO_WITHIN_OBB_MATRIX(point [x,y,z], center [x,y,z], half_extents [x,y,z], matrix 9 col-major) → BOOL
static void execFunc(const std::vector<std::shared_ptr<ValueVector>>& parameters,
    ...
    for (auto i = 0u; i < resultSelVector->getSelSize(); ++i) {
        ...
        if (!extractListDouble(*parameters[0], *parameterSelVectors[0], i, point, 3) ||
            ...
            !extractMat3(*parameters[3], *parameterSelVectors[3], i, mat)) {
...
function_set GeoWithinObbMatrixFunction::getFunctionSet() {
    function_set result;
