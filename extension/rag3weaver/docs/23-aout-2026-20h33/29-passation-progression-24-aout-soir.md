...
| Publication lucivy **3.0.0** | préparée sur `wip/publication-3.0.0` (doc 40, renuméroté 2.1.0 → 3.0.0 par lucivy) ; à la publication, nos path deps deviennent `lucivy-core = "3"`, `sparse-vector = "0.3"`, `luciole = "0.2"` — attend le go de Lucie |
...
2. **org id / project id.** ~~Décision d'architecture~~ prise (doc 37 : deux axes orthogonaux, index par cellule, `Scope` sur le `Catalog`) ; ~~A/B/C/D/E/F~~ **livrées** (`e2e_scope` 9/9, FFI `rag3weaver_catalog_set_scope`/`get_scope` + `scope`/`scopes`/`filterCondition`/`filters` lus depuis JS, README). **Chantier 2 terminé.** Trois bugs trouvés en chemin (doc 37 §6) dont **kuzu qui ignore la projection dans `QUERY_VECTOR_INDEX`** — canari en place.
3. ~~**Cross-encoder** (reranking)~~ **livré** : trait `Reranker`, `SearchOptions.rerank` (pool rescoré avant pagination), `BurnMiniLmReranker` = `cross-encoder/ms-marco-MiniLM-L-6-v2` sur burn (parité candle max |Δ| 5,7e-6 sur les logits, classements identiques), burnpack publié `Lucie666/ms-marco-minilm-l6-v2-burnpack`. Suites `e2e_rerank` (3, mock) et `e2e_burn_reranker` (5). **Multilingue livré aussi** : `BurnMMiniLmReranker` = `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1` (470 Mo, 14 langues, parité 1,3e-5) — défaut multilingue — et `BurnBgeRerankerV2M3` = `BAAI/bge-reranker-v2-m3` (2,2 Go, ~100 langues, parité 6,7e-6, Apache-2.0) ; burnpacks `Lucie666/mmarco-mminilmv2-l12-h384-v1-burnpack` et `Lucie666/bge-reranker-v2-m3-burnpack`. `e2e_burn_xlmr_reranker` (8, EN/FR/croisé). **Dense multilingue livré** : `BurnMultilingualMiniLmEmbedder` = `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` (470 Mo, 50+ langues, parité candle cosine ≥ 0,9999999, `e2e_burn_multilingual_minilm` 5 dont FR→EN sans recouvrement lexical ; les tests « multilingual » d'`e2e_search` tournent dessus) ; burnpack `Lucie666/paraphrase-multilingual-minilm-l12-v2-burnpack`. **Quatre burnpacks publiés ce soir, empreintes vérifiées après téléchargement.** Ouvert : blend score fusion × score reranker (le reranker remplace le score).
