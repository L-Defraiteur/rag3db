...
2. **org id / project id.** ~~Décision d'architecture~~ prise (doc 37 : deux axes orthogonaux, index par cellule, `Scope` sur le `Catalog`) ; ~~A/B/C/D~~ livrées (`e2e_scope` 9/9) ; reste E (FFI) et F (docs/README). Trois bugs trouvés en chemin (doc 37 §6) dont **kuzu qui ignore la projection dans `QUERY_VECTOR_INDEX`** — canari en place.
