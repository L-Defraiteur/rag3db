...
use crate::bm42_model::{Bm42Model, Config as Bm42Config};
...
/// Dual embedder that produces dense + sparse from a single BERT forward pass.
///
/// Uses [`Bm42Model`] internally (which returns both hidden states and attention
/// probs), then extracts:
/// - **Dense**: mean pooling of hidden states + L2 normalize (like [`CandleEmbedder`])
/// - **Sparse**: CLS attention weights (like [`Bm42Embedder`])
pub struct CandleDualEmbedder {
    model: Bm42Model,
    tokenizer: Mutex<Tokenizer>,
    dim: usize,
    device: Device,
...
impl CandleDualEmbedder {
    fn new_inner(
        ...
        let model = Bm42Model::load(vb, &config)
            .map_err(|e| EmbedError::ProviderError(format!("load dual model: {e}")))?;
        Ok(Self {
            model,
            tokenizer: Mutex::new(tokenizer),
            dim,
    ...
    /// Load a pre-configured model from HuggingFace Hub.
    #[cfg(feature = "candle-embedder")]
    pub fn new(default_model: DefaultModel) -> Result<Self, EmbedError> {
        Self::from_repo(default_model.model_id(), None)
    }
    ...
    pub fn from_repo(model_id: &str, revision: Option<&str>) -> Result<Self, EmbedError> {
        ...
        let config: Bm42Config = serde_json::from_str(
            &std::fs::read_to_string(&config_path)
                .map_err(|e| EmbedError::ProviderError(format!("read config: {e}")))?,
        ).map_err(|e| EmbedError::ProviderError(format!("parse config: {e}")))?;
    ...
    /// Load from in-memory bytes (WASM-compatible).
    pub fn from_bytes(
        config_json: &[u8],
        tokenizer_json: &[u8],
        weights: Vec<u8>,
    ) -> Result<Self, EmbedError> {
        let config: Bm42Config = serde_json::from_slice(config_json)
            .map_err(|e| EmbedError::ProviderError(format!("config parse: {e}")))?;
        let tokenizer = Tokenizer::from_bytes(tokenizer_json)
            .map_err(|e| EmbedError::ProviderError(format!("tokenizer parse: {e}")))?;
        let device = Device::Cpu;
    ...
    /// Single forward pass → dense + sparse embeddings.
    pub fn embed_dual_sync(&self, texts: &[String]) -> Result<(Vec<Vec<f32>>, Vec<SparseVector>), EmbedError> {
        ...
        // Single forward pass → (hidden_states, attention_probs)
        let (hidden_states, attn_probs) = self.model
        ...
        for i in 0..batch_size {
            let vec = normalized.get(i)
                .and_then(|t| t.to_vec1::<f32>())
                .map_err(|e| EmbedError::ProviderError(format!("extract dense: {e}")))?;
            dense.push(vec);
        }

        // ── Sparse: CLS attention weights ──
        let avg_attn = attn_probs.mean(1)
            .map_err(|e| EmbedError::ProviderError(format!("mean heads: {e}")))?;
        let cls_attn = avg_attn.narrow(1, 0, 1)
            .and_then(|t| t.squeeze(1))
            .map_err(|e| EmbedError::ProviderError(format!("cls row: {e}")))?;

        let mut sparse = Vec::with_capacity(batch_size);
        for i in 0..batch_size {
            let weights: Vec<f32> = cls_attn.get(i)
                .and_then(|t| t.to_vec1::<f32>())
                .map_err(|e| EmbedError::ProviderError(format!("extract sparse: {e}")))?;
            ...
            for (pos, (&token_id, &is_special)) in ids.iter().zip(special_mask.iter()).enumerate() {
            ...
            sparse.push(SparseVector::new(indices, values));
        }
