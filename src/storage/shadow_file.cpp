...
void ShadowFile::applyShadowPages(ClientContext& context) const {
    const auto pageBuffer = std::make_unique<uint8_t[]>(RAG3DB_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        ...
        dataFileInfo->writeFile(pageBuffer.get(), RAG3DB_PAGE_SIZE,
...
void ShadowFile::replayShadowPageRecords(ClientContext& context) {
    ...
    const auto headerBuffer = std::make_unique<uint8_t[]>(RAG3DB_PAGE_SIZE);
    shadowFileInfo->readFromFile(headerBuffer.get(), RAG3DB_PAGE_SIZE, 0);
    ...
    reader->resetReadOffset((header.numShadowPages + 1) * RAG3DB_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        shadowFileInfo->readFromFile(pageBuffer.get(), RAG3DB_PAGE_SIZE,
            shadowPageIdx * RAG3DB_PAGE_SIZE);
        dataFileInfo->writeFile(pageBuffer.get(), RAG3DB_PAGE_SIZE,
            record.originalPageIdx * RAG3DB_PAGE_SIZE);
        shadowPageIdx++;
    }
...
void ShadowFile::flushAll(main::ClientContext& context) const {
    ...
    writer->setFileOffset(shadowingFH->getNumPages() * RAG3DB_PAGE_SIZE);
