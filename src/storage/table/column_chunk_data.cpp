...
void ColumnChunkData::initializeScanState(SegmentState& state, const Column* column) const {
    ...
    if (residencyState == ResidencyState::ON_DISK) {
        ...
        state.numValuesPerPage = state.metadata.compMeta.numValues(RAG3DB_PAGE_SIZE, dataType);
...
uint64_t ColumnChunkData::getSizeOnDisk() const {
    ...
    return metadata.getNumDataPages(dataType.getPhysicalType()) * common::RAG3DB_PAGE_SIZE + nullSize;
}
