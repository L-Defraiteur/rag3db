...
ColumnChunkMetadata uncompressedFlushBuffer(std::span<const uint8_t> buffer, FileHandle* dataFH,
    ...
    KU_ASSERT(buffer.size_bytes() <= entry.numPages * RAG3DB_PAGE_SIZE);
...
ColumnChunkMetadata CompressedFlushBuffer::operator()(std::span<const uint8_t> buffer,
    ...
    const auto compressedBuffer = std::make_unique<uint8_t[]>(RAG3DB_PAGE_SIZE);
    auto numPages = 0u;
    const auto numValuesPerPage = metadata.compMeta.numValues(RAG3DB_PAGE_SIZE, dataType);
    ...
    while (valuesRemaining > 0) {
        const auto compressedSize = alg->compressNextPage(bufferStart, valuesRemaining,
            compressedBuffer.get(), RAG3DB_PAGE_SIZE, metadata.compMeta);
        ...
        if (compressedSize < RAG3DB_PAGE_SIZE) {
            memset(compressedBuffer.get() + compressedSize, 0, RAG3DB_PAGE_SIZE - compressedSize);
        }
    ...
    if (!dataFH->isInMemoryMode() && numPages < entry.numPages) {
        memset(compressedBuffer.get(), 0, RAG3DB_PAGE_SIZE);
...
std::pair<std::unique_ptr<uint8_t[]>, uint64_t> flushCompressedFloats(const CompressionAlg& alg,
    ...
    while (valuesRemaining > 0) {
        ...
        (void)castedAlg.compressNextPageWithExceptions(bufferCursor,
            ...
            RAG3DB_PAGE_SIZE, EncodeExceptionView<T>{exceptionBufferCursor},
