...
using rag3db_parquet::format::CompressionCodec;
using rag3db_parquet::format::ConvertedType;
using rag3db_parquet::format::Encoding;
using rag3db_parquet::format::PageType;
using rag3db_parquet::format::Type;

ColumnReader::ColumnReader(ParquetReader& reader, LogicalType type,
    const rag3db_parquet::format::SchemaElement& schema, idx_t fileIdx, uint64_t maxDefinition,
    uint64_t maxRepeat)
...
std::unique_ptr<ColumnReader> ColumnReader::createReader(ParquetReader& reader,
    common::LogicalType type, const rag3db_parquet::format::SchemaElement& schema, uint64_t fileIdx,
...
void ColumnReader::prepareRead(parquet_filter_t& /*filter*/) {
    ...
    defineDecoder.reset();
    block.reset();
    rag3db_parquet::format::PageHeader pageHdr;
...
void ColumnReader::decompressInternal(rag3db_parquet::format::CompressionCodec::type codec,
    ...
    case CompressionCodec::SNAPPY: {
        {
            ...
            auto res = rag3db_snappy::GetUncompressedLength(reinterpret_cast<const char*>(src),
        ...
        auto res = rag3db_snappy::RawUncompress(reinterpret_cast<const char*>(src), srcSize,
    ...
    case CompressionCodec::ZSTD: {
        auto res = rag3db_zstd::ZSTD_decompress(dst, dstSize, src, srcSize);
        // LCOV_EXCL_START
        if (rag3db_zstd::ZSTD_isError(res) || res != (size_t)dstSize) {
    ...
    case CompressionCodec::LZ4_RAW: {
        auto res = rag3db_lz4::LZ4_decompress_safe(reinterpret_cast<const char*>(src),
...
void ColumnReader::preparePageV2(rag3db_parquet::format::PageHeader& pageHdr) {
...
void ColumnReader::preparePage(rag3db_parquet::format::PageHeader& pageHdr) {
    auto& trans = reinterpret_cast<ThriftFileTransport&>(*protocol->getTransport());
...
void ColumnReader::prepareDataPage(rag3db_parquet::format::PageHeader& pageHdr) {
