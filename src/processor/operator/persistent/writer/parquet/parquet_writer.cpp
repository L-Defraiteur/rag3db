...
ParquetWriter::ParquetWriter(std::string fileName, std::vector<common::LogicalType> types,
    std::vector<std::string> columnNames, rag3db_parquet::format::CompressionCodec::type codec,
    ...
    rag3db_apache::thrift::protocol::TCompactProtocolFactoryT<ParquetWriterTransport> tprotoFactory;
    ...
    fileMetaData.created_by = "RAG3DB";

    fileMetaData.schema.resize(1);

    // populate root schema object
    fileMetaData.schema[0].name = "rag3db_schema";
    fileMetaData.schema[0].num_children = this->types.size();
    fileMetaData.schema[0].__isset.num_children = true;
    fileMetaData.schema[0].repetition_type = rag3db_parquet::format::FieldRepetitionType::REQUIRED;
