...
std::unique_ptr<ColumnWriter> ColumnWriter::createWriterRecursive(
    std::vector<rag3db_parquet::format::SchemaElement>& schemas, ParquetWriter& writer,
    ...
    case LogicalTypeID::STRUCT: {
        ...
        rag3db_parquet::format::SchemaElement schema_element;
    ...
    case LogicalTypeID::LIST: {
        ...
        rag3db_parquet::format::SchemaElement optionalElem;
        ...
        rag3db_parquet::format::SchemaElement repeatedElem;
    ...
    case LogicalTypeID::MAP: {
        ...
        // 	}
        // }
        rag3db_parquet::format::SchemaElement topElement;
        ...
        // key_value element
        rag3db_parquet::format::SchemaElement kv_element;
...
void ColumnWriter::compressPage(common::BufferWriter& bufferedSerializer, size_t& compressedSize,
    ...
    case CompressionCodec::SNAPPY: {
        compressedSize = rag3db_snappy::MaxCompressedLength(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        rag3db_snappy::RawCompress(reinterpret_cast<const char*>(bufferedSerializer.getBlobData()),
            bufferedSerializer.getSize(), reinterpret_cast<char*>(compressedBuf.get()),
            &compressedSize);
        compressedData = compressedBuf.get();
        KU_ASSERT(compressedSize <= rag3db_snappy::MaxCompressedLength(bufferedSerializer.getSize()));
    } break;
    case CompressionCodec::ZSTD: {
        compressedSize = rag3db_zstd::ZSTD_compressBound(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        compressedSize = rag3db_zstd::ZSTD_compress((void*)compressedBuf.get(), compressedSize,
    ...
    case CompressionCodec::LZ4_RAW: {
        compressedSize = rag3db_lz4::LZ4_compressBound(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        compressedSize = rag3db_lz4::LZ4_compress_default(
            reinterpret_cast<const char*>(bufferedSerializer.getBlobData()),
