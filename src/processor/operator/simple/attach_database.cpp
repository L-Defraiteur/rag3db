...
void AttachDatabase::executeInternal(ExecutionContext* context) {
    ...
    if (common::StringUtils::getUpper(attachInfo.dbType) == common::ATTACHED_RAG3DB_DB_TYPE) {
        auto db = std::make_unique<main::AttachedRag3dbDatabase>(attachInfo.dbPath,
            attachInfo.dbAlias, common::ATTACHED_RAG3DB_DB_TYPE, client);
