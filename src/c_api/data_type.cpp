...
void rag3db_data_type_create(rag3db_data_type_id id, rag3db_logical_type* child_type,
    uint64_t num_elements_in_array, rag3db_logical_type* out_data_type) {
...
void rag3db_data_type_clone(rag3db_logical_type* data_type, rag3db_logical_type* out_data_type) {
    out_data_type->_data_type =
        new LogicalType(static_cast<LogicalType*>(data_type->_data_type)->copy());
}

void rag3db_data_type_destroy(rag3db_logical_type* data_type) {
...
bool rag3db_data_type_equals(rag3db_logical_type* data_type1, rag3db_logical_type* data_type2) {
    return *static_cast<LogicalType*>(data_type1->_data_type) ==
           *static_cast<LogicalType*>(data_type2->_data_type);
}

rag3db_data_type_id rag3db_data_type_get_id(rag3db_logical_type* data_type) {
    auto data_type_id_u8 =
        static_cast<uint8_t>(static_cast<LogicalType*>(data_type->_data_type)->getLogicalTypeID());
    return static_cast<rag3db_data_type_id>(data_type_id_u8);
}

rag3db_state rag3db_data_type_get_num_elements_in_array(rag3db_logical_type* data_type,
    uint64_t* out_result) {
