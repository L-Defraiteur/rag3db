...
struct ParquetOptions {
    rag3db_parquet::format::CompressionCodec::type codec =
        rag3db_parquet::format::CompressionCodec::SNAPPY;
    ...
    void setCompression(common::Value& value) {
        ...
        if (strVal == "UNCOMPRESSED") {
            codec = rag3db_parquet::format::CompressionCodec::UNCOMPRESSED;
        } else if (strVal == "SNAPPY") {
            codec = rag3db_parquet::format::CompressionCodec::SNAPPY;
        } else if (strVal == "ZSTD") {
            codec = rag3db_parquet::format::CompressionCodec::ZSTD;
        } else if (strVal == "GZIP") {
            codec = rag3db_parquet::format::CompressionCodec::GZIP;
        } else if (strVal == "LZ4_RAW") {
            codec = rag3db_parquet::format::CompressionCodec::LZ4_RAW;
        } else {
