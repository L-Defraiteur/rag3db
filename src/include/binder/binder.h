...
public:
    ...
    RAG3DB_API std::unique_ptr<BoundStatement> bind(const parser::Statement& statement);

    RAG3DB_API std::shared_ptr<Expression> createVariable(const std::string& name,
        const common::LogicalType& dataType);
    RAG3DB_API std::shared_ptr<Expression> createInvisibleVariable(const std::string& name,
        const common::LogicalType& dataType) const;
    RAG3DB_API expression_vector createVariables(const std::vector<std::string>& names,
        const std::vector<common::LogicalType>& types);
    RAG3DB_API expression_vector createInvisibleVariables(const std::vector<std::string>& names,
    ...
    RAG3DB_API std::shared_ptr<NodeExpression> createQueryNode(const std::string& parsedName,
        const std::vector<catalog::TableCatalogEntry*>& entries);
    ...
    /*** validations ***/
    RAG3DB_API static void validateTableExistence(const main::ClientContext& context,
        const std::string& tableName);
    RAG3DB_API static void validateNodeTableType(const catalog::TableCatalogEntry* entry);
    RAG3DB_API static void validateColumnExistence(const catalog::TableCatalogEntry* entry,
    ...
    RAG3DB_API void addToScope(const std::string& name, std::shared_ptr<Expression> expr);
