...
LogicalType RAG3DB_API inferMinimalTypeFromString(const std::string& str);
LogicalType RAG3DB_API inferMinimalTypeFromString(std::string_view str);
...
void RAG3DB_API castStringToBool(const char* input, uint64_t len, bool& result);
...
template<typename T, bool IS_SIGNED = true>
RAG3DB_API inline bool trySimpleIntegerCast(const char* input, uint64_t len, T& result) {
...
RAG3DB_API inline void simpleIntegerCast(const char* input, uint64_t len, T& result,
...
inline bool tryDoubleCast(const char* input, uint64_t len, T& result) {
    ...
    auto end = input + len;
    auto parse_result = rag3db_fast_float::from_chars(input, end, result);
