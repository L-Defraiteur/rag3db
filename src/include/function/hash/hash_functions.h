...
inline void Hash::operation(const std::string_view& key, common::hash_t& result) {
    ...
    for (size_t i = 0u; i < key.size() / 8; i++) {
        auto blockHash = rag3db::function::murmurhash64(*(data64 + i));
        hashValue = rag3db::function::combineHashScalar(hashValue, blockHash);
    }
    uint64_t last = 0;
    for (size_t i = 0u; i < key.size() % 8; i++) {
        last |= static_cast<uint64_t>(key[key.size() / 8 * 8 + i]) << i * 8;
    }
    hashValue = rag3db::function::combineHashScalar(hashValue, rag3db::function::murmurhash64(last));
    result = hashValue;
}
