...
public:
    ColumnReader(ParquetReader& reader, common::LogicalType type,
        const rag3db_parquet::format::SchemaElement& schema, common::idx_t fileIdx,
    ...
    static std::unique_ptr<ColumnReader> createReader(ParquetReader& reader,
        common::LogicalType type, const rag3db_parquet::format::SchemaElement& schema,
    ...
    void decompressInternal(rag3db_parquet::format::CompressionCodec::type codec, const uint8_t* src,
        uint64_t srcSize, uint8_t* dst, uint64_t dstSize);
    void preparePageV2(rag3db_parquet::format::PageHeader& pageHdr);
    void preparePage(rag3db_parquet::format::PageHeader& pageHdr);
    void prepareDataPage(rag3db_parquet::format::PageHeader& pageHdr);
...
protected:
    const rag3db_parquet::format::SchemaElement& schema;
    ...
    const rag3db_parquet::format::ColumnChunk* chunk = nullptr;
