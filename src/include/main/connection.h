...
public:
     ...
     */
    RAG3DB_API explicit Connection(Database* database);
    /**
     * @brief Destructs the connection.
     */
    RAG3DB_API ~Connection();
    /**
     ...
     */
    RAG3DB_API void setMaxNumThreadForExec(uint64_t numThreads);
    /**
     ...
     */
    RAG3DB_API uint64_t getMaxNumThreadForExec();
     ...
     */
    RAG3DB_API std::unique_ptr<QueryResult> query(std::string_view query);

    RAG3DB_API std::unique_ptr<QueryResult> queryAsArrow(std::string_view query, int64_t chunkSize);
     ...
     */
    RAG3DB_API std::unique_ptr<PreparedStatement> prepare(std::string_view query);
     ...
     * @return the prepared statement.
     */
    RAG3DB_API std::unique_ptr<PreparedStatement> prepareWithParams(std::string_view query,
     ...
     * @return the result of the query.
     */
    RAG3DB_API std::unique_ptr<QueryResult> executeWithParams(PreparedStatement* preparedStatement,
        std::unordered_map<std::string, std::unique_ptr<common::Value>> inputParams);
    /**
     * @brief interrupts all queries currently executing within this connection.
     */
    RAG3DB_API void interrupt();

    /**
     * @brief sets the query timeout value of the current connection. A value of zero (the default)
     * disables the timeout.
     */
    RAG3DB_API void setQueryTimeOut(uint64_t timeoutInMS);
...
private:
    ...
    RAG3DB_API void addScalarFunction(std::string name, function::function_set definitions);
    RAG3DB_API void removeScalarFunction(std::string name);
