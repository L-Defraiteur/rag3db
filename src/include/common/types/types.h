...
struct RAG3DB_API internalID_t {
    offset_t offset;
    table_id_t tableID;
...
class LogicalType {
    ...
    RAG3DB_API LogicalType(const LogicalType& other);

public:
    RAG3DB_API LogicalType() : typeID{LogicalTypeID::ANY}, extraTypeInfo{nullptr} {
        physicalType = getPhysicalType(this->typeID);
    };
    explicit RAG3DB_API LogicalType(LogicalTypeID typeID, TypeCategory info = TypeCategory::INTERNAL);
    EXPLICIT_COPY_DEFAULT_MOVE(LogicalType);

    RAG3DB_API bool operator==(const LogicalType& other) const;
    RAG3DB_API bool operator!=(const LogicalType& other) const;
    ...
    RAG3DB_API LogicalTypeID getLogicalTypeID() const { return typeID; }
    bool containsAny() const;
    bool isInternalType() const { return category == TypeCategory::INTERNAL; }

    RAG3DB_API PhysicalTypeID getPhysicalType() const { return physicalType; }
    RAG3DB_API static PhysicalTypeID getPhysicalType(LogicalTypeID logicalType,
    ...
    RAG3DB_API static std::vector<LogicalType> copy(const std::vector<LogicalType>& types);
    RAG3DB_API static std::vector<LogicalType> copy(const std::vector<LogicalType*>& types);
    ...
    static RAG3DB_API LogicalType DECIMAL(uint32_t precision, uint32_t scale);
    ...
    static RAG3DB_API LogicalType STRUCT(std::vector<StructField>&& fields);

    static RAG3DB_API LogicalType RECURSIVE_REL(std::vector<StructField>&& fields);

    static RAG3DB_API LogicalType NODE(std::vector<StructField>&& fields);

    static RAG3DB_API LogicalType REL(std::vector<StructField>&& fields);

    static RAG3DB_API LogicalType UNION(std::vector<StructField>&& fields);

    static RAG3DB_API LogicalType LIST(LogicalType childType);
    template<class T>
    static inline LogicalType LIST(T&& childType) {
        return LogicalType::LIST(LogicalType(std::forward<T>(childType)));
    }

    static RAG3DB_API LogicalType MAP(LogicalType keyType, LogicalType valueType);
    template<class T>
    ...
    static RAG3DB_API LogicalType ARRAY(LogicalType childType, uint64_t numElements);
    template<class T>
...
private:
    ...
    friend class rag3db::processor::ParquetReader;
...
class RAG3DB_API ExtraTypeInfo {
public:
...
class RAG3DB_API UDTTypeInfo : public ExtraTypeInfo {
public:
...
class RAG3DB_API ListTypeInfo : public ExtraTypeInfo {
public:
...
class RAG3DB_API ArrayTypeInfo final : public ListTypeInfo {
public:
...
struct RAG3DB_API LogicalTypeUtils {
