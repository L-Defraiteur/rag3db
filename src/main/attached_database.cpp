...
void AttachedDatabase::invalidateCache() {
    if (dbType != common::ATTACHED_RAG3DB_DB_TYPE) {
    ...
    if (vfs->fileOrPathExists(walFilePath, context)) {
        ...
        if (walFile->getFileSize() > 0) {
            throw common::RuntimeException(common::stringFormat(
                "Cannot attach an external Rag3db database with non-empty wal file. Try manually "
                "checkpointing the external database (i.e., run \"CHECKPOINT;\")."));
        }
    }
}

AttachedRag3dbDatabase::AttachedRag3dbDatabase(std::string dbPath, std::string dbName,
    ...
    if (DBConfig::isDBPathInMemory(dbPath)) {
        throw common::RuntimeException("Cannot attach an in-memory Rag3db database. Please give a "
                                       "path to an on-disk Rag3db database directory.");
    }
    ...
    if (!vfs->fileOrPathExists(path, clientContext)) {
        throw common::RuntimeException(common::stringFormat(
            "Cannot attach a remote Rag3db database due to invalid path: {}.", path));
    }
